...
wolfkde <- rbind(wolfused, wolfavail) #can rbind bc columns are in same order
str(wolfkde)
...
bvkde<- subset(wolfkde, subset=pack =="Bow Valley") ### could just do factor
par(mfrow = c(2,3))
...
# Tibble can't compute a mean if there are NAs
...
rbinom(100, 1, 0.5)   #r simulates - think 'results' of binom
...
```{r}
plogis(0)
## note that if p = 0, then the probability of an outcome is 0.5, heads or tails - i.e., a fair coin - logit scale centers around zero
plogis(1)
plogis(-5)
plogis(-100)
plogis(5)
...
y = rbinom(length(x), 1, plogis(1 - 0.01*x) )
...
summary(elev)
# intercept means plogis scale probability at 0
...
## interpreting things in probability is not useful bc you don't know the true absolute probability - odds express relative relationships bw probaility of success or failure
...
elev <- glm(used ~ Elevation2, family=binomial(logit), data=wolfkde)
summary(elev)
wolfkde$fitted.Elev <- fitted(elev) #prob of use based on elevation
head(wolfkde)
...
ggplot(wolfkde, aes(x = fitted.Elev, y=after_stat(density), fill=usedFactor)) + geom_histogram(binwidth=0.05, position="identity", alpha=0.7) + xlab("Predicted Probability of Wolf Use") + theme(axis.title.x=element_text(size=16)) + facet_grid(pack ~ ., scales="free") #density histo bc pack sample sizes are different
```
...
## Plotting Logistic Regresson with ggplot2 - DO THIS IN PRACTICE
...
figPrey #the problem with this is that it makes it looks like a multiple regression but really it's just multiple univariate regressions graphed on top of each other
...
tiff("Output/figPrey.tif", width = 6, height =6, units = "in", res = 300)
figPrey
dev.off()
```
